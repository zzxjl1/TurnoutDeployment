...
RENDER_POOL_MAX_TASKS_PER_PROC = 200  # 渲染进程最大复用次数（None为无限次）
...
RESTART_ON_OOM = True # 检测到oom时自动重启服务
...
LOG_ROTATION_POLICY = "TIME" # 分片策略（“TIME” or “SIZE”）
LOG_ROTATION_MAX_TIME_INTERVAL = 1 # 分片时钟周期,仅当分片策略为TIME时生效
LOG_ROTATION_MAX_TIME_UNIT = "D" # 分片时钟单位（S：Seconds、M：Minutes、H：Hours、D：Days）
LOG_ROTATION_MAX_SIZE = 50  # 文件分片大小（MB）,仅当分片策略为SIZE时生效
LOG_BACKUP_COUNT = 7  # 文件滚动备份数量
